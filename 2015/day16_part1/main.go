...
func FindSue(sues []Sue, mfcsam MFCSAM) (int, error) {
	...
	for i := range sues {
		...
		for key, val := range sues[i].Attributes {
			...
			case "cats":
				score += calcScore(uintAbs(val, mfcsam.Cats))
			case "samoyeds":
				score += calcScore(uintAbs(val, mfcsam.Samoyeds))
			case "pomeranians":
				score += calcScore(uintAbs(val, mfcsam.Pomeranians))
			case "akitas":
				score += calcScore(uintAbs(val, mfcsam.Akitas))
			case "vizslas":
				score += calcScore(uintAbs(val, mfcsam.Vizslas))
			case "goldfish":
				score += calcScore(uintAbs(val, mfcsam.Goldfish))
			case "trees":
				score += calcScore(uintAbs(val, mfcsam.Trees))
			case "cars":
