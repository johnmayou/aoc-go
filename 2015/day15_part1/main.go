...
func FindBestRecipe(ingredients []Ingredient, totalTeaspoons int) (map[string]int, int) {
	...
	maxScore := 0
	var dfs func(i int, teaspoons int, recipe map[string]int)
	dfs = func(i int, teaspoons int, recipe map[string]int) {
		if i == len(ingredients)-1 {
			recipe[ingredients[i].Name] = totalTeaspoons - teaspoons // set remaining tsps to last ingredient
		...
		for tsps := range totalTeaspoons - teaspoons + 1 {
			recipe[name] = tsps
			dfs(i+1, teaspoons+tsps, recipe)
		}
	}
	dfs(0, 0, make(map[string]int, len(ingredients)))
...
func main() {
	...
	recipe, score := FindBestRecipe(ingredients, 100)
